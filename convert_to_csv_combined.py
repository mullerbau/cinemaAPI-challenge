#!/usr/bin/env python3
"""
Script para converter casos de teste para CSV com formato combinado
ID + Nome no campo Resumo para Jira em português
"""

import csv

def extract_test_cases_combined():
    """Extrai todos os casos de teste com formato combinado"""
    
    # Dados dos casos de teste com formato: ID + Nome combinados
    test_cases = []
...
def create_csv_combined():
    """Cria arquivo CSV com formato combinado para Jira em português"""
    
    # Cabeçalho do CSV em português
    headers = ["Resumo", "Descrição", "Camada", "Módulo", "Tipo", "Prioridade"]
    
    # Obter casos de teste
    test_cases = extract_test_cases_combined()
    
    # Criar arquivo CSV
    with open('test_cases_jira_combined.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    print(f"Arquivo CSV combinado criado com {len(test_cases)} casos de teste!")
    print("Arquivo: test_cases_jira_combined.csv")
    print("Formato: ID + Nome combinados no campo Resumo")
    print("Pronto para import no Jira em português!")
